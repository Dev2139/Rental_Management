...
const SharePropertyButton = ({ propertyId, propertyTitle, propertyImage, className = '' }: SharePropertyButtonProps) => {
  const [copied, setCopied] = useState(false);
  
  const currentUrl = window.location.origin + `/property/${propertyId}`;
  ...
  const shareLinks = {
    ...
    email: `mailto:?subject=${encodedTitle}&body=Check out this property: ${currentUrl}`,
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className={`w-10 h-10 rounded-full bg-card/90 backdrop-blur flex items-center justify-center hover:bg-card transition-colors ${className}`}>
